...
// RunTest provides a mock function with given fields: dagID, envFile, settingsFile, startDate, containerName, taskLogs
func (_m *ImageHandler) RunTest(dagID string, envFile string, settingsFile string, startDate string, containerName string, taskLogs bool) error {
	ret := _m.Called(dagID, envFile, settingsFile, startDate, containerName, taskLogs)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, string, string, string, string, bool) error); ok {
		r0 = rf(dagID, envFile, settingsFile, startDate, containerName, taskLogs)
	} else {
		r0 = ret.Error(0)
	}
