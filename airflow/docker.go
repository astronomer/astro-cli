...
const (
	...
	composeLinkWebserverMsg = "Airflow Dashboard is available at %s"
	composeLinkPostgresMsg  = "Postgres Database is available at %s"
...
func DockerComposeInit(airflowHome, envFile, dockerfile, imageName string) (*DockerCompose, error) {
	...
	//TODO: If verbose show the compose output
...
func (d *DockerCompose) Start(imageName, settingsFile, composeFile, buildSecretString string, noCache, noBrowser bool, waitTime time.Duration, envConns map[string]astrocore.EnvironmentObjectConnection) error {
	...
	if imageName == "" {
		...
		imageBuildErr := d.imageHandler.Build(d.dockerfile, buildSecretString, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false, NoCache: noCache})
		if !config.CFG.DisableAstroRun.GetBool() {
	...
	s.Suffix = " Project is starting up…"
	s.Start()
	defer s.Stop()
...
func (d *DockerCompose) Stop(waitForExit bool) error {
	s := spinner.New(runtimes.SpinnerCharSet, runtimes.SpinnerRefresh) // Use a character set and speed
	s.Suffix = " Stopping project…"
	s.Start()
	defer s.Stop()
	...
	if !waitForExit {
		s.FinalMSG = ansi.Green("\u2714") + " Project has been stopped.\n"
		s.Stop()
		return nil
	}
			...
			for i := range psInfo {
				...
				if strings.Contains(psInfo[i].Name, PostgresDockerContainerName) {
					if psInfo[i].State == dockerExitState {
						...
						s.FinalMSG = ansi.Green("\u2714") + " Airflow has been stopped.\n"
						s.Stop()
						return nil
					}
...
func (d *DockerCompose) Kill() error {
	...
	s.FinalMSG = ansi.Green("\u2714") + " Project has been killed.\n"
	s.Stop()
...
func (d *DockerCompose) Pytest(pytestFile, customImageName, deployImageName, pytestArgsString, buildSecretString string) (string, error) {
	...
	if deployImageName == "" {
		// build image
		if customImageName == "" {
			err := d.imageHandler.Build(d.dockerfile, buildSecretString, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false})
			if err != nil {
				return "", err
			}
		} else {
	...
	// run pytests
	exitCode, err := d.imageHandler.Pytest(pytestFile, d.airflowHome, d.envFile, "", pytestArgs, false, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false})
	if err != nil {
		return exitCode, err
	}
...
func (d *DockerCompose) UpgradeTest(newAirflowVersion, deploymentID, newImageName, customImage, buildSecretString string, conflictTest, versionTest, dagTest bool, astroPlatformCore astroplatformcore.CoreClient) error {
	...
	if deploymentID != "" {
		...
		imageBuildErr := d.imageHandler.Build(d.dockerfile, buildSecretString, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false})
		if imageBuildErr != nil {
			return imageBuildErr
		}
	}
...
func (d *DockerCompose) conflictTest(testHomeDirectory, newImageName, newAirflowVersion string) error {
	...
	exitCode, conflictErr := d.imageHandler.ConflictTest(d.airflowHome, testHomeDirectory, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false})
	if conflictErr != nil {
		return conflictErr
	}
...
func (d *DockerCompose) versionTest(testHomeDirectory, currentAirflowVersion, deploymentImage, newDockerFile, newAirflowVersion, customImage, buildSecretString string) error {
	...
	imageBuildErr := d.imageHandler.Build(newDockerFile, buildSecretString, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false})
	if imageBuildErr != nil {
		return imageBuildErr
	}
...
func (d *DockerCompose) dagTest(testHomeDirectory, newAirflowVersion, newDockerFile, customImage, buildSecretString string) (int, error) {
	...
	exitCode, err := d.imageHandler.Pytest(pytestFile, d.airflowHome, d.envFile, testHomeDirectory, strings.Fields(htmlReportArgs), true, airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false})
	if err != nil {
...
func (d *DockerCompose) RunDAG(dagID, settingsFile, dagFile, executionDate string, noCache, taskLogs bool) error {
	...
	err = d.imageHandler.Build(d.dockerfile, "", airflowTypes.ImageBuildConfig{Path: d.airflowHome, Output: false, NoCache: noCache})
	if err != nil {
		return err
	}
