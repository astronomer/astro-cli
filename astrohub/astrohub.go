...
// Client containers the logger and HTTPClient used to communicate with the AstrohubAPI
type Client struct {
	HTTPClient *httputil.HTTPClient
}

// NewAstrohubClient returns a new Client with the logger and HTTP client setup.
func NewAstrohubClient(c *httputil.HTTPClient) *Client {
	return &Client{
		HTTPClient: c,
	}
}
...
func (r *Request) Do() (*Response, error) {
	return r.DoWithClient(NewAstrohubClient(httputil.NewHTTPClient()))
}
...
func (c *Client) Do(doOpts httputil.DoOptions) (*Response, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Failed to JSON decode Astrohub response")
	}
