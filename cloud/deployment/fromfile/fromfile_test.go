...
`
			...
			fileutil.WriteStringToFile(filePath, data)
			defer afero.NewOsFs().Remove(filePath)
			mockCoreClient.On("ListClustersWithResponse", mock.Anything, orgID, clusterListParams).Return(&mockListClustersResponse, nil).Once()
			err = CreateOrUpdate("deployment.yaml", "create", mockClient, mockCoreClient, nil)
...
`
			...
			fileutil.WriteStringToFile(filePath, data)
			defer afero.NewOsFs().Remove(filePath)
			mockCoreClient.On("ListClustersWithResponse", mock.Anything, orgID, clusterListParams).Return(&astrocore.ListClustersResponse{}, errTest).Once()
			err = CreateOrUpdate("deployment.yaml", "create", mockClient, mockCoreClient, nil)
