...
func TestCreate(t *testing.T) {
	...
	t.Run("success with standard/dedicated type different scheduler sizes", func(t *testing.T) {
		...
		err = Create("test-name", ws, "test-desc", csID, "4.2.5", dagDeploy, KUBERNETES, gcpCloud, "us-west-2", LargeScheduler, "enable", "", "", "", "", "", astroplatformcore.DeploymentTypeSTANDARD, 0, 0, mockPlatformCoreClient, mockCoreClient, false)
		assert.NoError(t, err)
