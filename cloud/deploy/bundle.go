...
func retrieveLocalGitMetadata(bundlePath string) (deployGit *astrocore.DeployGit, commitMessage string) {
	...
	default:
		logger.Logger.Debugf("Unsupported Git provider, skipping Git metadata retrieval: %s", repoURL.Host)
		return nil, ""
	}
	urlPath := strings.TrimPrefix(repoURL.Path, "/")
	firstSlashIndex := strings.Index(urlPath, "/")
	if firstSlashIndex == -1 {
		logger.Logger.Debugf("Failed to parse remote repository path, skipping Git metadata retrieval: %s", repoURL.Path)
		return nil, ""
	}
	...
	if err != nil {
		logger.Logger.Debugf("Failed to retrieve local repository path prefix, skipping Git metadata retrieval: %s", err)
		return nil, ""
	}
	if path != "" {
		deployGit.Path = &path
