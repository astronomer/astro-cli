...
type ErrInvalidReleaseName struct {
	errs []string
}

func (e ErrInvalidReleaseName) Error() string {
	strErrs := strings.Join(e.errs, "\n\t")
	return fmt.Sprintf("the release name you provided is invalid:\n\t%s", strErrs)
}
...
func validateReleaseName(releaseName string) error {
	...
	if len(releaseName) > maxCharacters {
	...
	if len(errs) > 0 {
		return ErrInvalidReleaseName{errs: errs}
	}
