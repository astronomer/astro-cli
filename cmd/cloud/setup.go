...
func checkAPIKeys(platformCoreClient astroplatformcore.CoreClient, isDeploymentFile bool) (bool, error) {
	...
	if !isDeploymentFile {
		...
		fmt.Println("\nWarning: Starting June 1st, 2024, we no longer would be supporting Deployment API Keys. To ensure uninterrupted access to our services, we strongly recommend transitioning to Deployment API tokens. See https://docs.astronomer.io/astro/deployment-api-tokens.")
	}
