...
func DeleteUsingWorkspaceUUID(serviceAccountID, workspaceID string, client houston.ClientInterface, out io.Writer) error {
	sa, err := client.DeleteWorkspaceServiceAccount(houston.DeleteServiceAccountRequest{ServiceAccountID: serviceAccountID, WorkspaceID: workspaceID})
	if err != nil {
		return err
	}
...
func DeleteUsingDeploymentUUID(serviceAccountID, deploymentID string, client houston.ClientInterface, out io.Writer) error {
	sa, err := client.DeleteDeploymentServiceAccount(houston.DeleteServiceAccountRequest{DeploymentID: deploymentID, ServiceAccountID: serviceAccountID})
	if err != nil {
		return err
	}
